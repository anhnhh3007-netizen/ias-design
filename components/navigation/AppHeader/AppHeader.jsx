...
export function AppHeader({ searchPlaceholder = 'Tìm kiếm', onSearch,
  ...
  return (
    <header className={className} style={{
      height: 64, background: '#fff', borderBottom: '1px solid #D9DBE9',
      display: 'flex', alignItems: 'center', padding: '0 24px', gap: 12, flexShrink: 0,
      fontFamily: 'var(--font-family-body, "Plus Jakarta Sans", sans-serif)',
      ...style,
    }} {...rest}>
      <div style={{ flex: 1, maxWidth: 420, position: 'relative', display: 'flex', alignItems: 'center' }}>
        <svg style={{ position: 'absolute', left: 10, color: '#A0A3BD', pointerEvents: 'none' }}
        ...
        <input value={search} onChange={e => { setSearch(e.target.value); onSearch && onSearch(e.target.value); }}
          ...
          style={{ width: '100%', height: 36, borderRadius: 8, border: '1px solid #D9DBE9',
            background: '#F4F6FA', padding: '0 12px 0 34px', fontSize: 13,
            fontFamily: 'inherit', color: '#4E4B66', outline: 'none' }} />
      </div>
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
        <button onClick={onNotification} style={{
          width: 36, height: 36, borderRadius: 8, border: 'none', background: 'transparent',
          cursor: 'pointer', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6E7191',
        }}>
          ...
          {notificationCount > 0 && (
            <span style={{ position: 'absolute', top: 4, right: 4, minWidth: 16, height: 16,
              background: '#EB2D4B', borderRadius: 8, border: '2px solid #fff',
        ...
        {user && (
          <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#141ED2',
